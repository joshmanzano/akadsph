...
const Requests = ({ className, pending, ...rest }) => {
  ...
  pending.forEach((request) => {
    ...
    request.available_days.forEach((available) => {
      ...
      const label =
        moment(start_date).format("MMM Do, h:mm a") +
        " - " +
        moment(end_date).format("h:mm a");
      const date = moment(start_date).format("MMM Do YYYY");
      const time =
        moment(start_date).format("h:mm a") +
        " - " +
        moment(end_date).format("h:mm a");
    ...
    const row = {
      ...
      duration:
        request.request.time + (request.request.time > 1 ? " hours" : " hour"),
      ...
      fileButton: (
        <Button
          ...
          href={request.request.extra_files}
          target="_blank"
        >
          Files
        </Button>

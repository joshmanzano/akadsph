...
const TopBar = ({
  ...
  const exampleNotif = {
    image: "../static/images/oli-happy.png",
    message:
      "Charles Samoy accepted your session request on December 3, 2020 for Nate Mercado",
    detailPage: "/",
    receivedTime: "5m ago",
    seen: false,
  };
        ...
        {["Overview", "Book A Tutor", "Buy Credits", "Profile", "Settings"].map(
          (text, index) => (
            <ListItem
              button
              key={text}
              onClick={() => window.location.replace(pagelinks[index])}
            >
  ...
  const data = [
    {
      image:
        "https://synergi-dev.s3.ap-southeast-1.amazonaws.com/profile-pictures/6b9.png",
      ...
      <Toolbar>
        ...
        >
          <Badge
            ...
            variant="dot"
            invisible={
              unreadCount == 0 || window.location.pathname != "/messages"
            }
          >
            <ForumIcon />
          </Badge>
        </IconButton>
        <IconButton color="inherit" onClick={toggleSeen}>
          <Badge color="secondary" variant="dot" invisible={seen}>
            <Notifications
              // data={data}
              data={notifications}
              markAsRead={(e) => {
